...
#include <hardware/sb16.h>
#include <hardware/isa.h>
...
static void sb_handler(int vector) {
    ...
    inb(SB_INTR16); // 读取中断状态寄存器以清除中断

    u8 state = inb(SB_STATE);

    LOGK("SB16 interrupt: state=0x%02x\n", state);
    if (sb->waiter) {
...
static void sb_reset(sb_t *sb) {
    outb(SB_RESET, 1); // 发送复位命令
    task_sleep(1);
    outb(SB_RESET, 0); // 结束复位

    u8 state = inb(SB_READ);
    LOGK("SB16 reset: state=0x%02x\n", state);
}
