...
static void path_push(char *path, const char *name)
{
    size_t len = strlen(path);
    if (strcmp(path, "/"))
        path[len++] = '/';

    strlcpy(path + len, name, MAX_PATH_LEN - len);
}
...
void abspath(char *pwd, const char *pathname)
{
    ...
    if (IS_SEPARATOR(pathname[0]))
        ...
        strlcpy(tmp, pwd, MAX_PATH_LEN);

    while (*pathname)
    {
        while (IS_SEPARATOR(*pathname))
            pathname++;
        ...
        int len = 0;
        while (pathname[len] && !IS_SEPARATOR(pathname[len]))
            len++;
