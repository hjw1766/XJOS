...
idx_t minix_ialloc(super_t *super) {
    ...
    for (size_t i = 0; i < desc->imap_blocks; i++) {
        ...
        bitmap_make(&map, buf->data, BLOCK_SIZE, i * BLOCK_BITS + 1);
        bit = bitmap_scan(&map, 1);
        if (bit != EOF) {
...
static inode_t *iget(dev_t dev, idx_t nr) {
    ...
    if (nr == 0 || nr > desc->inodes) {
        LOGK("iget bad inode dev=%d nr=%d max=%d\n", dev, nr, desc->inodes);
    }
...
static inode_t *new_inode(dev_t dev, idx_t nr) {
    ...
    minode->mtime = inode->atime = inode->mtime = inode->ctime = sys_time();
    minode->nlinks = 1;
...
static int minix_write(inode_t *inode, char *data, int len, off_t offset) {
    ...
    // 更新修改时间
    minode->mtime = inode->atime = sys_time();
...
static buffer_t *add_entry(inode_t *dir, const char *name, minix_dentry_t **result) {
    ...
    for (; true; i++, entry++) {
        ...
        buf->dirty = true;
        dir->mtime = minode->mtime = sys_time();
        dir->buf->dirty = true;
        *result = entry;
        return buf;
    };
...
int minix_rmdir(inode_t *dir, char *name) {
    ...
    dminode->nlinks--;
    dir->ctime = dir->atime = dminode->mtime = sys_time();
    dir->buf->dirty = true;
