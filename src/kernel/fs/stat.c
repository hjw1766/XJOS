...
static void copy_stat(inode_t *inode, stat_t *statbuf) {
    ...
    statbuf->rdev = inode->desc->zones[0]; // assuming rdev stored in zones[0]
...
int sys_fstat(fd_t fd, stat_t *statbuf) {
    ...
    if (!file)
        return EOF; // File descriptor not open

    inode_t *inode = file->inode;
    if (!inode)
        return EOF; // No inode associated with file
