...
typedef struct e1000_t {
    ...
    pci_device_t *device; // PCI设备信息
    u32 membase; // 内存基地址

    u8 mac[6]; // MAC地址
    bool link; // 网线连接状态
    bool eeprom; // 是否有EEPROM

    rx_desc_t *rx_descs; // 接收描述符数组
    u16 rx_cur; // 当前接收描述符索引
...
static void recv_packet(e1000_t *e1000) {
    while (true) {
        rx_desc_t *rx = &e1000->rx_descs[e1000->rx_cur];
        // 如果描述符状态没有DD位，说明这个描述符还没有被网卡写回，说明没有数据包了
        if (!(rx->status & RS_DD)) {
            return;
        }

        if (rx->errors) {
            LOGK("errror 0x%X happened...\n", rx->errors);
        }
        ...
        e1000->rx_cur = (e1000->rx_cur + 1) % RX_DESC_NR; // 移动到下一个描述符
    }
};
